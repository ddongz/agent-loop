...
```sh
git remote add github <GITHUB_GIT_URL>
git remote add nju <NJU_GIT_URL>
git push -u github feature-sentinelloop
git push -u nju feature-sentinelloop

gh pr create --repo <GITHUB_OWNER>/<GITHUB_REPOSITORY> --base main --head feature-sentinelloop --title "Release SentinelLoop v0.1.0" --body "Local release evidence is in RELEASE_CHECKLIST.md."
glab mr create --repo <NJU_NAMESPACE>/<NJU_REPOSITORY> --source-branch feature-sentinelloop --target-branch main --title "Release SentinelLoop v0.1.0" --description "Local release evidence is in RELEASE_CHECKLIST.md."
```

PR/MR 合并且远程 CI 全绿后，才在已同步的 release commit 上执行：

```sh
git switch main
git pull --ff-only github main
git tag -a v0.1.0 -m "SentinelLoop v0.1.0"
git push github v0.1.0
git push nju v0.1.0
```

GitHub 的 tag push 将触发 Release workflow。真实仓库 URL、CI pass、Release URL 均尚不存在，不能写成已完成。

## 7. 唯一学生内容门

学生本人创建 `REFLECTION.md` 并撰写 1500–2500 个中文汉字；本次提交不得包含 AI 生成的实质反思。完成后在 PowerShell 复验：

```powershell
if (-not (Test-Path -LiteralPath 'REFLECTION.md')) { throw 'REFLECTION.md missing' }
$reflection = Get-Content -Raw -Encoding UTF8 'REFLECTION.md'
$hanCount = ([regex]::Matches($reflection, '[\p{IsCJKUnifiedIdeographs}\p{IsCJKUnifiedIdeographsExtensionA}]')).Count
if ($hanCount -lt 1500 -or $hanCount -gt 2500) { throw "REFLECTION.md Han count out of range: $hanCount" }
```
