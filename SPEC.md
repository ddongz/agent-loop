...
| 路径逃逸 | realpath/规范化后验证工作区前缀和符号链接；每次写前复验；平台支持时 no-follow 打开；原子替换 | v1 以单仓库、单 Agent、单写者为前提；纯 Node 路径 API 不能提供基于目录句柄的全链路原子检查，恶意并发替换父目录的 OS/文件系统 TOCTOU 仍属残余风险 |
